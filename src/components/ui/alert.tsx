...
const alertVariants = cva(
  "group/alert relative grid w-full gap-1 rounded-[1.5rem] border px-4 py-4 text-left text-sm has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-3 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-5",
  {
    variants: {
      variant: {
        default: "border-border/80 bg-card text-card-foreground",
        info: "border-primary/18 bg-primary/8 text-foreground",
